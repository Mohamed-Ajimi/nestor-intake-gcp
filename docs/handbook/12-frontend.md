...
That is 136 `it()` declarations in source. The STATE.md ledger records vitest reporting 140 passing on 2026-08-31 (`.planning/STATE.md:561-562`), and **the four-test difference is arithmetic, not a discrepancy**: `funnelLabels.test.ts:222` puts two `it()` declarations inside `for (const [lng, catalog] of CATALOGS)`, and `CATALOGS` holds the three locale catalogues (`:215-219`). Two static declarations therefore register six tests at collection time, so 136 − 2 + 6 = **140**. Both numbers are right; only one of them is countable with grep. What no test covers: the auth guard, the SSE readers, the `useRunEvents` delta path (its own comment says "inspected, not tested"), `ProposalListControl`'s write-back, `NextStepBanner`, `VerificationReport`, the detail page. See [15 — Quality and testing](15-quality-and-testing.md).

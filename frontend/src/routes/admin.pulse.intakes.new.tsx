...
function NewIntakePage() {
 ...
 const submit = async () => {
 const errs: string[] = [];
 if (!clientName.trim()) errs.push("Geef een klantnaam op.");
 setErrors(errs);
 ...
 De intake staat klaar op status 'Concept'. Open ze om de antwoorden in te vullen.
 </p>
 {created.client_name && (
 <p className="mt-4 font-mono text-xs uppercase tracking-wider text-ink/60">
 Klant: {created.client_name}
 </p>
 )}
 ...
 Geef de klant een naam. De intake wordt aangemaakt binnen de actieve klant-ruimte.
 </p>
 ...
 Klantnaam <span className="font-normal text-ink/60">*</span>
 </Label>
 <Input
 id="client_name"
 value={clientName}

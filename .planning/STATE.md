...
|---|-------------|------|--------|-----------|
| 260925-dyt | Raw-output zip keeps every angle report: `research/NN-<question>-<provider>.md` + header + `research/index.md` (was one file per provider — 2–3 of up to 15 reports survived unzip on all 13 prod bundles). Backend 916 passed / 2 skipped (not Docker). Existing prod bundles NOT rebuilt. **DEV 2026-09-25**: `nestor-api-00055-gx8` = `backend:51210b5` sha256:9950670c… (build fb763ce0, idle gate 8c4f41c6 passed, readyz 200); tribunal untouched (flags on). Prod pending the dev acceptance run. | 2026-09-25 | 9cb5394 | [260925-dyt-raw-output-zip-one-file-per-angle-named-](./quick/260925-dyt-raw-output-zip-one-file-per-angle-named-/) |
| 260925-fqt | Zip headers + index show the FULL question (expanded from the report's chapter heading; engine label stays 120). 919 passed / 2 skipped. | 2026-09-25 | 4535cf4 | [260925-fqt-full-question-text-in-zip-headers](./quick/260925-fqt-full-question-text-in-zip-headers/) |
| 20260909-trace | Grouped light-mode researcher trace with stable task/pass IDs, provider states, raw/audit fallback and EN/NL/FR labels. 262 frontend + 207 targeted engine tests passed; tsc/lint/build and desktop/mobile fixture checks passed. **DEPLOYED 09:22Z** — `nestor-frontend-00040-tgk` / `tribunal-api-20260909-091435-111943` / `tribunal-worker-20260909-091435-112037`; gates re-run by the orchestrator (262 fe, 207+13 engine, i18n PASS); revert tags `state-before/after-trace-visuals-260909`. ⛔ trace_* meta not yet observed on a live run. | 2026-09-09 | 7188b7e | [20260909-grouped-research-trace](./quick/20260909-grouped-research-trace/) |
| 260909-g8b | **The new trace design is the whole run page.** `7188b7e` shipped the grouped shell for `deep_research` only, gated on trace meta — opening a run showed the old feed. Split `ResearchTrace` into `TraceSection` (shell) + `ResearchTraceBody`; every `FeedGroup` renders the shell from the first event (divider→title, summary→subline, no duplicate rows); grouping is the body only where executions exist; `EmptyFeed` in a shell; status card container matched. RED first (`data-trace-section` 0→2). 266 tests, tsc 0, i18n PASS, build ✓. ⚠ partial-revert-via-worker property is GONE; rollback = `nestor-frontend-00040-tgk`. ⛔ never seen in a browser. **DEPLOYED** `nestor-frontend-00041-g9j`. | 2026-09-09 | 3992d13 | [260909-g8b-run-page-new-design-every-phase](./quick/260909-g8b-run-page-new-design-every-phase/) |
...
| 260907-eyd | **DEF-23.2-12 + DEF-23.2-13 — the client's validation-phase choice now persists, and the submit verb enforces completeness.** 2 commits `21acffd`/`e03b2c9`. ⭐ **THE REGISTER WAS WRONG ON THREE POINTS AND EACH CHANGED THE FIX.** (1) *Not one line — TWO gates.* `handleChange` returned before `setDirtyFields`, so nothing was ever marked dirty; fixing `saveCurrentSection` alone (the recorded fix) would have changed NOTHING because `dirtyKeys` stays empty. (2) *The headline defect was the wrong one.* "Keep Nestor's proposal" (`onConfirm`) only collapses a diff card to a badge — it never changes an answer, so nothing was lost by not persisting it. The real loss was `extra_questions_proposed`, the ONE enabled control in the validation phase: the client's choice of AI-proposed extra research questions was discarded while the status advanced, so those questions never reached the research run. This is the other half of `260831-gk7`, which made the checkbox live but left it unsaved. (3) ⛔ **THE NAIVE FIX WOULD HAVE BEEN WORSE THAN THE BUG.** Deleting both early-returns marks the REVERT button's fields dirty too (`decision_or_goal`/`audience_description`/`company_intro`/`output_size`/`output_form`, none of them `proposal_list`), the server refuses those in the validation phase with **409** (D-23.2-05), `saveCurrentSection` returns false and `doSubmit` gates on it — **a silent discard becomes a permanent inability to submit.** So the rule was NARROWED, not removed, and extracted to one predicate (`lib/intake-writable.ts`) now driving `handleChange`, `saveCurrentSection` AND the `disabled=` prop. The prop's inline expression was *equivalent* to the new predicate; it was a SECOND copy and the save paths held a disagreeing THIRD — enabled and saved are now the same question, which is the actual defect. **DEF-23.2-13** adds `check_submit_completeness` as a SEPARATE function, not a relaxation of `check_answer_batch`'s ⛔ block (that prohibition is about the per-save path and still holds — the form saves only dirty fields and clears fields routinely). ⚠ **`draft -> submitted` ONLY**: `POST /submit` serves two transitions and enforcing on `reviewed -> validated_by_client` is a LOCKOUT — the client has one writable field in that phase so a refused client cannot comply, and the answer set is the ADMIN's work by then. `test_route_validate_transition_is_exempt_from_completeness` exists to make that "obvious" widening go red. Emptiness mirrors the browser EXACTLY (None/""/empty list, deliberately not `{}`): a server stricter than the form refuses a submit the form said was fine, so this one yields. **Three pre-existing tests went red — correctly, and predicted before running** (3 failed / 36 passed): each created a bare intake and submitted it expecting 200. Fixed as FIXTURE updates, not assertion changes — each now fills the form as a real client does, via a new `complete_answer_batch` fixture DERIVED from the canonical schema so a future required field widens it automatically. The client-surface pin (row 6) mattered most: it asserts EXACTLY 200 to prove the route is OPEN, so a 422 there would be a **false authorization signal**. Backend **736 → 764 passed** / 2 skipped / 0 failed (+28, closes exactly); frontend tsc 0, **154 → 161** vitest, i18n-audit PASS; route inventory **61 gate-bearing / 26 gated, /intakes 43/26 — IDENTICAL** (neither change moves a gate). **Recorded, not fixed:** the revert affordance is inert BY DESIGN (the server refuses those writes; whether a client may reject a refinement needs an operator ruling); `handleChange` writes localStorage before the writable check so a revert *looks* persisted until the cache clears; and `ValidationDiff.tsx:211` keys on `research_questions`, which is NOT one of the 29 canonical field keys — the refined-question diff cards therefore never render in the client form. ⛔ **Still zero browser/E2E tests** — DEF-23.2-12 is exactly that defect class: an enabled control whose value is dropped was invisible to tsc, 161 vitest assertions, 764 backend tests and the i18n audit, all green while it shipped | 2026-09-07 | e03b2c9 | [20260907-validation-writes-submit-completeness](./quick/20260907-validation-writes-submit-completeness/) |
...
| 260907-uq4 | **Codify Cloud SQL backup + PITR config in `infra/main.tf`.** Backups + PITR were enabled by hand on `nestor-pg` 2026-09-07 (it had ZERO backups); the `backup_configuration` block had never been declared, so a `terraform apply` would have reverted production to no backups. Values match the live read-back so `plan` shows no diff. ⭐ MEASURED: PITR on Cloud SQL for **PostgreSQL** needs NO instance restart — the earlier note saying it did is preserved and marked corrected, not deleted. ⛔ Still open: no restore ever rehearsed; instance still ZONAL (no HA). | 2026-09-07 | bdc6268 | [260907-uq4-codify-cloud-sql-backup-configuration-in](./quick/260907-uq4-codify-cloud-sql-backup-configuration-in/) |
| 260908-m7j | **DEF-23.2-16 — AI-refined answers rendered as `[object Object]`.** The review panel persists localized `{nl,fr,en}` objects into `value_json`; all three `value_json` read boundaries handed the object straight to the renderers. Added `resolveAnswerValue`, delegating locale choice to the existing `pick()` (whose `undefined` return is the discriminator that keeps `{choice,text}` and stakeholder rows intact). Resolved on READ so the already-broken intake is repaired with no migration. 19 new tests, 180 total, tsc clean. ⚠ accepted: a later save of a touched field collapses that field's object to one language. ⛔ never verified in a browser. | 2026-09-08 | eeb79ea | [260908-m7j-resolve-localized-answer-values-at-the-f](./quick/260908-m7j-resolve-localized-answer-values-at-the-f/) |
| 260908-vno | **DEF-23.2-16b — the client saw no diff cards at all.** `ValidationDiff`'s `typeof p.suggested !== "string"` guard rejected the skill's `{nl,fr,en}` objects, so `getSimpleProposal` returned `null` for every proposal. Resolved the proposals payload at its load boundary in `IntakeForm`; `ValidationDiff` deliberately untouched. 193 tests. ⭐ ROOT CAUSE of both of today's defects is `260831-lm4` (2026-08-31), which changed the skill output contract to nl+fr+en and swept only FOUR consumers — one of which (`NestorBriefingPDF`) has since been deleted, and two of which were never covered. **The contract is pinned by no test.** ⛔ diff cards NOT yet confirmed live. | 2026-09-08 | 6a7d3a1 | [260908-vno-resolve-localized-proposals-so-validatio](./quick/260908-vno-resolve-localized-proposals-so-validatio/) |
| 260909-168 | **DEF-23.3-01 — healthy runs reclaimed as dead at exactly 60 min; the liveness heartbeat never lands.** Measured: both concurrent runs reclaimed at claim+3600.20s/+3600.22s (same 200ms offset), zero of ~120 heartbeats, zero `run_heartbeat_failed`. Consequences observed: a FINISHED report silently discarded by the ownership fence (rowcount 0, no log); 108 skeptic sessions re-billed; claims ACCUMULATE across attempts (734 = 345+389). Shipped OBSERVABILITY ONLY — `run_heartbeat_started` + `run_heartbeat rowcount=` — plus STALE 60->90 as a STOPGAP (deploy-worker.sh AND variables.tf). Did NOT guess a fix. 44 passed/0 skipped vs Postgres. Executor's leading hypothesis: the task DIES to an uncaught `CancelledError`, not starvation — timestamps decide. Owed: start ONE run and read the log; stop discarding finished reports; make resume idempotent; ~25 docs still say 60. **DEPLOYED** `tribunal-worker-20260908-233038-013724`. | 2026-09-08 | 5d9c0ae | [260909-168-heartbeat-rowcount-diagnostic-and-stale](./quick/260909-168-heartbeat-rowcount-diagnostic-and-stale/) |
| 260909-3ow | **ROOT CAUSE of DEF-23.3-01 — the worker ran as TWO module copies with TWO `WORKER_ID`s.** `CMD python -m runs.worker` made worker.py `__main__`; execute.py's lazy `from …runs.worker import execute_run` loaded a second copy. The claim stamped one id, every D-23.1-06 fenced write (heartbeat, parks, finalize, failure) bound the other → all matched 0 rows, finished reports silently discarded, rows stuck `running`. Latent since 07-20 (Phase 13); armed by 23.1-05's fence on 09-04 (`2c02c26`, `6722560`); first live 09-07; broke on the first real run 09-08. Tests could not see it (canonical import = one copy). Fix: `worker_main.py` launcher + Dockerfile CMD + `sys.modules` self-alias + a runpy structural test (RED verbatim: `…-1f22a40c` vs `…-3d2aebf6`). ⚠ corrects 09-08: the 'Run complete + spinner' was a REJECTED completion write, not a label collision. **DEPLOYED** `tribunal-worker-20260909-005403-025920` 01:00Z. ✅ `rowcount=1` OBSERVED live 06:37Z on run 6668b27e, wid = a booted worker id. ✅ run 6668b27e COMPLETED 07:40Z with a saved report — first on any fenced build. STALE 90→60 revert now unblocked. | 2026-09-09 | c80c2e3 | [260909-3ow-worker-single-import-launcher-fixes-fence](./quick/260909-3ow-worker-single-import-launcher-fixes-fence/) |

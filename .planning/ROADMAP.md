...
- [x] 23-02-PLAN.md — wave 2 — UAT-22-F4: the enumerated run-status to work-phase presentation rule, the five honest banner bodies, and the end of the run-research sentence
...
**Wave 3**
- [x] 23.6-05-PLAN.md — ship to DEV only: gates, operator-run build + nestor-migrate (0018) + nestor-api + nestor-frontend, revision-digest read-back, deploy record, browser walkthrough (no tribunal deploy, no prod)
...
2. One research run started, observed in logs and browser, and COMPLETED on `nestor-pulse-prod` with a saved report — before any client user is created (D-23.4-06).
3. A release tag promotes the dev digests to `nestor-pulse-prod` without rebuilding; the worker step exits non-zero while any tribunal `run` row is `status='running'`; the smoke step checks `/readyz`, `/auth/login` and one intake load.
4. `tribunal/infrastructure/cloud-run/*.sh` no longer default to `project-cb01b861-cb4a-438d-b9a`; a missing project fails the script.
5. STALE back to 60 in `deploy-worker.sh` AND `infra/variables.tf`; the attempt cap excludes `cancelled`; the frontend surfaces `needs_investigation` (D-23.4-07).
...
- [x] 23.5-02-PLAN.md — archive wiring + DELETE /intakes/{id} with the research-run 409 and the GCS prefix wall
- [x] 23.5-03-PLAN.md — project/client naming in nl/fr/en + the AI context header (label-only, D-23.5-03)
